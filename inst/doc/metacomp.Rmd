...
`B1` to try to figure out how that difference came about.  The `all.equal`
output is unhelpful in this respect.
...
two vectors are actually similar, at least in the part of output that is not
truncated.  With `diffPrint` it is obvious that `B2` is the same as `A2`, except
