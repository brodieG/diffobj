...
This is a slightly simplified version of the YAML I use for this R Markdown
document.
...
Depending on what mechanism you are using to render your Rmd documents into
HTML, you may or may not need to specify the YAML.

Additionally, any R chunks should include the `results='asis'` option, e.g.:
...
Although you may want to do the following to preserve the vignette or whatever
other CSS you wish to use in combination with `diffobj`'s:
...
old.opts <- options(
  diffobj.format="html",
  diffobj.style=list(html.output="diff.only")   # notice this changed
)
