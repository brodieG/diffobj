...
If you look closely at the next matching line you will notice that the `a` and `b` values are not exactly the same.  The row indeces are different, but `diffobj` excludes row indices from the diff so that rows that are identical otherwise are shown as matching.  `diffobj` indicates this is happening by showing the portions of a line that are ignored in the diff in grey.
...
which may cause the diff to render funny if you change screen widths, etc., between the two steps.
...
While the parameter list is extensive, only the objects being compared are required and all other parameters have default values, and most of them are for advanced used only.  The defaults all reference `diffobj.*` options so that you can change them if you prefer different ones.
...
* "ansi256" for ANSI 256 color formatting
