...
If you look closely at the next matching line you will notice they are not quite the same.  The row indeces are different, but `diffobj` excludes row indices from the diff so that rows that are identical otherwise are shown as matching.  `diffobj` indicates this is happening by showing the portions of a line that are ignored in the diff in grey.
...
This method of comparing vectors is a compromise that may in some circumstances produce sub-optimal results.  You can also use `diffChr` (see below) to do a direct element by element comparison.
...
* `diffCsv`: loads two CSV files into data frames and compare the data frames with `diffPrint`
* `diffDeparse`: deparses and compares the character vectors produced by the depasing
...
All the `diff*` methods return a `Diff` S4 object.  It has a `show` method which is responsible for rendering the `Diff` and displaying it to the screen.  Because you can compute and render diffs in two steps:
...
It is possible for the diff to render funny if you change screen widths, etc., in between the time the diff is computed and the time it is rendered.
...
While the parameter list is extensive, only the objects being compared are required and all other parameters have hopefully sensible defaults.  The defaults all reference `diffobj.*` options so that you can change them if you dislike them.
...
If your terminal supports it `diffobj` will format with ANSI escape sequences.  `diffobj` uses Gábor Csárdi's [`crayon`](https://github.com/gaborcsardi/crayon) package to detect ANSI support and to apply ANSI based formatting.  If your terminal does not support colors, `diffobj` will attempt to output an HTML/CSS formatted diff to your browser using `browseURL`.
...
At this time the only `format` that is affected by this parameter is `brightness`.  If you want to specify your own light/dark/neutral schemes you may do so either by specifying a [style](#styles) directly or with [Palette of Styles](#styles).

### Pagers

In interactive mode, if the diff output is very long `diff*` methods will pipe output to a pager. The default action is to create a temporary file with the diff output, and feed that to `file.show`.  If your terminal does not support ANSI escape sequences, the default action is to use your web browser as a pager / terminal irrespective of how big the actual output is.
...
This algorithm scales with the _square_ of the number of _differences_.

Unless you have a diff with many (> 10K) differences, odds are that the slow step in the process will be the post-processing this package does to generate the display representation of the diff rather than the diff itself.

## Acknowledgements

* R Core for creating and publishing R, a wonderful programming language
* Eugene Myers for the diff algorithm
* Michael B. Allen for implementing it in C
* Gábor Csárdi for adding a whole new dimension to the R terminal experience with the `crayon` package
* Jim Hester for `covr`, and also for unwittingly sowing the seed that sprouted into this package
* Hadley Wickham for the `devtools` and `testthat` packages
